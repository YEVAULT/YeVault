...
function resetHero(){
  ...
  document.getElementById('hero-ye').style.display='block';
}
function setHeroArt(era){
  const url=allData.art[era];
  const artEl=document.getElementById('hero-art');
  const yeEl=document.getElementById('hero-ye');
  if(url){
    artEl.src=url;
    artEl.style.display='block';
    yeEl.style.display='none';
  } else {
    artEl.style.display='none';
    yeEl.style.display='block';
  }
...
function renderEras(eraObj){
  ...
  keys.forEach(era=>{
    ...
    const artUrl=allData.art[era]||'';
    const thumbHtml=artUrl
      ?`<img class="era-thumb" src="${artUrl}" alt="" loading="lazy" onerror="this.style.display='none'">`
      :`<div class="era-thumb-placeholder"></div>`;
    row.innerHTML=`<div class="era-row-left">${thumbHtml}<div class="era-row-name">${era}</div></div><div class="era-row-count">${songs.length} ${currentLabel}</div>`;
